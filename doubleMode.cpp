...
doubleMode::doubleMode(int length, int width, bool is_loading, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::doubleMode)
{
    ui->setupUi(this);
    ...
    this->setWindowTitle("QLink: 双人模式");
    ...
    connect(timer1, &QTimer::timeout, [=] ()
    {
        ...
        if (left == 0 || !isMore()) {
            ...
            if (mark > mark2) {
                label->move(30, 30);
                label->setText("玩家 1 获得胜利！");
            }

            if (mark < mark2) {
                label->move(30, 30);
                label->setText("玩家 2 获得胜利！");
            }
    ...
    connect(timer2, &QTimer::timeout, [=] ()
    {
        ...
        // 同样需要对于第二个用户的选择进行处理
        if (one.ID_x == 0 && one.ID_y == 0) {
            for (int var = 0; var < 20; ++var) {
                if (x2 == p[var].ID_x && y2 == p[var].ID_y) {
                    one.ID_x = x2;
                    one.ID_y = y2;
                    ONE = var;
                }
            ...
            if (one.ID_x != x2 || one.ID_y != y2) {
                if (two.ID_x == 0 && two.ID_y == 0) {
                    for (int var = 0; var < 20; ++var) {
                        if (x2 == p[var].ID_x && y2 == p[var].ID_y) {
                            two.ID_x = x2;
                            two.ID_y = y2;
        ...
        if (ONE != -1 && TWO != -1) {
            if (isSame2 && LinkMatch(p[ONE].ID_x, p[ONE].ID_y, p[TWO].ID_x, p[TWO].ID_y)) {
                p[ONE].ID_x = 0;
                p[ONE].ID_y = 0;
                p[TWO].ID_x = 0;
                p[TWO].ID_y = 0;  // 横纵坐标均为零意味着绘制时从屏幕中消失
                mark2 ++;
                QString num = QString::number(mark2);
                ui->scores_2->setText(num);
            }
...
// 键盘操作移动方向
void doubleMode::keyPressEvent(QKeyEvent *event)
{
...
void doubleMode :: paintEvent(QPaintEvent *)
{
    // 首先绘制 RPG 角色
    ...
    player.drawPixmap(x2 * 100 - 100, y2 * 100 - 100, 100, 100, QPixmap(":/image/character2.png"));
    ...
    // 将选中的方块进行高亮
    QBrush brush2(QColor(0, 0, 255, 130));
    player.setBrush(brush2);
    player.drawEllipse(QPoint((first.ID_x * 100 - 50), (first.ID_y * 100 - 50)), 50, 50);
    player.drawEllipse(QPoint((second.ID_x * 100 - 50), (second.ID_y * 100 - 50)), 50, 50);

    QBrush brush3(QColor(0, 0, 255, 130));
    player.setBrush(brush2);
    player.drawEllipse(QPoint((one.ID_x * 100 - 50), (one.ID_y * 100 - 50)), 50, 50);
    player.drawEllipse(QPoint((two.ID_x * 100 - 50), (two.ID_y * 100 - 50)), 50, 50);
}

// 该函数主要用于实现道具能够提供的功能
void doubleMode::Tools()
{
    // +1s: 延长剩余时间 30s
    ...
    if ((x == shuffle.ID_x && y == shuffle.ID_y) ||
            (x2 == shuffle.ID_x && y2 == shuffle.ID_y)) {
        // 首先处理所有剩余的方块
        for (int var = 0; var < 20; ++var) {
...
// 判断两个方块能否直接通过直线连接
bool doubleMode::straightLinkMatch(int x1, int y1, int x2, int y2)
{
...
// 判断两个方块能否通过仅有一折的折线连接
bool doubleMode::inTwoLines(int x1, int y1, int x2, int y2) {
    // 矩形的两条边上没有方块阻挡
...
// 判断两个方块能否通过两折的折线连接
bool doubleMode::inThreeLines(int x1, int y1, int x2, int y2) {
...
bool doubleMode::LinkMatch(int x1, int y1, int x2, int y2)
{
...
void doubleMode::save()
{
    ...
    if (file.open(QIODevice::ReadWrite)) {
        ...
        // 记录第二个玩家的相关信息
        stream << QString::number(mark2) << endl;
        stream << QString::number(x2) << endl;
        stream << QString::number(y2) << endl;
...
void doubleMode::load()
{
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        // 重载得到第二个玩家的相关信息
        mark2 = in.readLine().toInt();
        x2 = in.readLine().toInt();
        y2 = in.readLine().toInt();
